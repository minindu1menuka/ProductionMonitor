...
import { Search, Plus, Download, Trash2, Save, Database, AlertCircle, RefreshCw } from 'lucide-react';
...
export function ProductionAnalysisReport() {
  ...
  const handleSearch = async () => {
    if (!sku) return alert("Please enter an SKU to search.");
    if (loadedSKUs.some(s => s.sku === sku)) return alert("This SKU is already loaded.");
  ...
  const handleCreateNewSKU = () => {
    ...
    if (!trimmedSku) return alert("Please enter a new SKU name.");
    if (loadedSKUs.some(s => s.sku === trimmedSku)) return alert("This SKU is already on the screen.");
  ...
  const handleDeleteSKU = async (skuIndex, skuName) => {
    const isConfirmed = window.confirm(`Are you sure you want to permanently delete the entire SKU '${skuName}'? This will remove all its items from the database.`);
    ...
    try {
      ...
      alert(`SKU '${skuName}' has been successfully deleted.`);
    } catch (error) {
      console.error("Error deleting SKU:", error);
      alert("An error occurred while deleting the SKU.");
    }
  };
  ...
  const exportToExcel = async () => {
    if (loadedSKUs.length === 0) return alert("No data available to export!");
    ...
    loadedSKUs.forEach((group) => {
      ...
      sheet.getCell(`C${currentRow}`).font = { bold: true, color: { argb: 'FFDC2626' } }; // Tailwind Red-600
      ...
      sheet.getCell(masterQtyCellRef).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFF1F5F9' } }; // Tailwind Slate-100
      currentRow += 2;
      ...
      group.items.forEach((item) => {
        ...
        [11, 12, 13].forEach(col => {
          row.getCell(col).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFFFFBEB' } }; // Tailwind Amber-50
    ...
    for (let i = 0; i < 4; i++) {
      ...
      dCell.font = { bold: true, color: { argb: 'FF166534' } }; // Tailwind Green-800
